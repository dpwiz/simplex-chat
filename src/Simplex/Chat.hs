...
processChatCommand = \case
  ...
  APISendMessage (ChatRef cType chatId) live itemTTL (ComposedMessage file_ quotedItemId_ mc) -> withUser $ \user@User {userId} -> withChatLock "sendMessage" $ case cType of
    ...
    where
      ...
      xftpSndFileTransfer user file fileSize n contactOrGroup = do
        ...
        aFileId <- withAgent $ \a -> xftpSendFile a (aUserId user) (error "wtf: fsFilePath") (roundedFDCount n)
  ...
  APIConnect userId incognito (Just (ACR SCMInvitation cReq)) -> withUserId userId $ \user -> withChatLock "connect" . procCmd $ do
    ...
    connId <- withAgent $ \a -> joinConnection a (aUserId user) True cReq True . directMessage $ XInfo profileToSend
  ...
  APIJoinGroup groupId -> withUser $ \user@User {userId} -> do
    ...
    withChatLock "joinGroup" . procCmd $ do
      agentConnId <- withAgent $ \a -> joinConnection a (aUserId user) True connRequest True . directMessage $ XGrpAcpt (memberId (membership :: GroupMember))
      withStore' $ \db -> do
    ...
    connectViaContact user@User {userId} incognito cReq@(CRContactUri ConnReqUriData {crClientData}) = withChatLock "connectViaContact" $ do
      ...
      withStore' (\db -> getConnReqContactXContactId db user cReqHash) >>= \case
        ...
        (_, xContactId_) -> procCmd $ do
          ...
          incognitoProfile <- if incognito then Just <$> liftIO generateRandomProfile else pure Nothing
          let profileToSend = userProfileToSend user incognitoProfile Nothing
          connId <- withAgent $ \a -> joinConnection a (aUserId user) True cReq True $ directMessage (XContact profileToSend $ Just xContactId)
...
receiveViaCompleteFD user fileId RcvFileDescr {fileDescrText, fileDescrComplete} =
  when fileDescrComplete $ do
    ...
    aFileId <- withAgent $ \a -> xftpReceiveFile a (aUserId user) rd (error "wtf: CryptoFileArgs")
...
processAgentMessageConn user@User {userId} corrId agentConnId agentMessage = do
  ...
  where
        ...
        GCHostMember -> do
          ...
          if isMember memId gInfo members
            then messageWarning "x.grp.mem.intro ignored: member already exists"
              ...
              groupConnIds <- createAgentConnectionAsync user CFCreateConnGrpMemInv enableNtfs SCMInvitation True
              directConnIds <- createAgentConnectionAsync user CFCreateConnGrpMemInv enableNtfs SCMInvitation True
    ...
    xGrpMemFwd gInfo@GroupInfo {membership, chatSettings = ChatSettings {enableNtfs}} m memInfo@(MemberInfo memId memRole _) introInv@IntroInvitation {groupConnReq, directConnReq} = do
      ...
      groupConnIds <- joinAgentConnectionAsync user enableNtfs groupConnReq True $ directMessage msg
      directConnIds <- joinAgentConnectionAsync user enableNtfs directConnReq True $ directMessage msg
      let customUserProfileId = if memberIncognito membership then Just (localProfileId $ memberProfile membership) else Nothing
...
agentAcceptContactAsync user enableNtfs autoSub invId msg = do
  ...
  connId <- withAgent $ \a -> acceptContactAsync a (aCorrId cmdId) enableNtfs autoSub invId $ directMessage msg
  pure (cmdId, connId)
