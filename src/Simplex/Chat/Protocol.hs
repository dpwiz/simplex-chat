...
parseMsgContainer v =
  MCQuote <$> v .: "quote" <*> mc
    ...
    <|> MCSimple <$> mc
  where
    mc = ExtMsgContent <$> v .: "content" <*> v .:? "file" <*> v .:? "ttl" <*> v .:? "live" <*> v .:? "direct"
...
msgContainerJSON = \case
  ...
  where
    o = JM.fromList
    msgContent (ExtMsgContent c file ttl live direct) = ("file" .=? file) $ ("ttl" .=? ttl) $ ("live" .=? live) $ ("direct" .=? direct) ["content" .= c]
